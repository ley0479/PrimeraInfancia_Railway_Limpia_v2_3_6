...
def test_schema_ddl_is_not_run_by_route_registration_or_requests() -> None:
    ...
    motor_routes = _text('backend/modules/motor_plantillas/routes.py')
    motor_repository = _text('backend/modules/motor_plantillas/repository.py')
    ...
    assert 'init_schema(database_path)' not in motor_routes
    assert 'init_minutas_schema(database_path)' not in motor_routes
    assert 'init_schema(database_path)' not in motor_repository


def test_runtime_guard_precedes_hosting_import_and_migrations_are_explicit() -> None:
    startup = _text('start_hosting.sh')
    predeploy = _text('predeploy_hosting.sh')
    init_hosting = _text('backend/init_hosting.py')

    assert 'python backend/init_hosting.py' in predeploy
    assert 'python backend/init_hosting.py' not in startup
    assert 'export SKIP_RUNTIME_SCHEMA_DDL=1' in startup
    ...
    assert 'migrate_unidades_tenant_unique(config_class.DATABASE_PATH)' in init_hosting
    assert 'init_motor_plantillas_schema(config_class.DATABASE_PATH)' in init_hosting
    assert 'init_rpp_minutas_schema(config_class.DATABASE_PATH)' in init_hosting


def test_motor_schema_is_portable_between_sqlite_and_postgresql() -> None:
    schema = _text('backend/modules/motor_plantillas/schema.py')
    services = _text('backend/modules/motor_plantillas/services.py')

    assert 'BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY' in schema
    assert "if database.is_sqlite:" in services
    assert "'INTEGER PRIMARY KEY AUTOINCREMENT'" in services
...
if __name__ == '__main__':
    ...
    test_motor_schema_is_portable_between_sqlite_and_postgresql()
