...
def run():
    ...
    with tempfile.TemporaryDirectory(prefix='pi-v253-') as temp:
        ...
        tasks=motor.list_tasks(1,{'periodo':'2026-08'},limit=5000)
        require(tasks,'El motor no consolidó fuentes')
        require(first['creadas']>0 and second['creadas']==0 and len(motor.list_tasks(1,{},limit=5000))==first['fuentes_leidas'],'Sincronización no idempotente')
        require(all(t['fundacion_id']==1 for t in tasks),'Se cruzaron tareas de otra fundación')
        require(any(t['fuente_modulo']=='GESTION_INTEGRAL_UCA' for t in tasks),'Falta Ruta Operativa')
        require(any(t['fuente_modulo']=='GESTION_PEDAGOGICA' for t in tasks),'Falta Gestión Pedagógica')
        require(any(t['fuente_modulo']=='SALUD_NUTRICION' for t in tasks),'Falta Salud y Nutrición')
    ...
    js=(ROOT/'frontend'/'js'/'modules'/'motor-gestion-proyecto.js').read_text()
    giujs=(ROOT/'frontend'/'js'/'modules'/'gestion-integral-uca.js').read_text()
