...
def main():
    with tempfile.TemporaryDirectory(prefix="pi_v241_") as temp_dir:
        ...
        assert_true("/api/acceso/ping" not in local_bat, "El inicio local aún usa ping autenticado")
        assert_true("/api/health" in local_bat and "/api/health" in tunnel_ps, "Falta healthcheck público")
        assert_true("CREDENCIALES_INICIALES_LOCAL.txt" in local_bat, "No se documentan credenciales iniciales aleatorias")
        assert_true("secrets.token_urlsafe(64)" in local_bat, "SECRET_KEY/JWT_SECRET_KEY locales no son aleatorias")
        assert_true("AdminLocal2026*" not in local_bat, "Persiste una contraseña local fija")
        assert_true("local-dev-secret-key" not in local_bat, "Persiste una SECRET_KEY local fija")
