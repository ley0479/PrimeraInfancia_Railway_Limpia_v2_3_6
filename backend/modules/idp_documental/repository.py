...
from .services import connect, init_schema, now_iso, public_document
...
class IDPRepository:
    ...
    def complete_extraction(self, document_id: int, tenant_id: int, raw: dict, canonical: dict, fields: list[dict], classification: tuple[str,float,str], user_id=None):
        ...
        conn = connect(self.database_path)
        conn.execute("""UPDATE idp_documentos SET tipo_documento=?,confianza_clasificacion=?,estado=?,etapa=?,progreso=?,motor_lectura=?,resultado_bruto_json=?,resultado_canonico_json=?,validaciones_json=?,fecha_actualizacion=? WHERE id=? AND fundacion_id=?""", (kind,confidence,status,'PENDIENTE_OCR' if needs_ocr else 'REVISION_HUMANA',65 if needs_ocr else 80,raw.get('motor'),json.dumps(raw,ensure_ascii=False,default=str),json.dumps(canonical,ensure_ascii=False,default=str),json.dumps([{'nivel':'ADVERTENCIA','codigo':'OCR_REQUERIDO','mensaje':'Conecte un motor OCR para continuar.'}] if needs_ocr else [],ensure_ascii=False),now,document_id,tenant_id))
        for field in fields:
    ...
    def correct_field(self, document_id: int, field_id: int, tenant_id: int, value: Any, user_id, reason=''):
        ...
        conn.execute("UPDATE idp_documentos SET resultado_canonico_json=?,fecha_actualizacion=? WHERE id=? AND fundacion_id=?",(json.dumps(canonical,ensure_ascii=False,default=str),now,document_id,tenant_id)); conn.commit(); conn.close()
    ...
    def approve(self, document_id: int, tenant_id: int, user_id):
        conn=connect(self.database_path); row=conn.execute("SELECT estado FROM idp_documentos WHERE id=? AND fundacion_id=?",(document_id,tenant_id)).fetchone()
